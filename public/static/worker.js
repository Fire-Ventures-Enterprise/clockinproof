...
async function saveNewPin() {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (m) m.style.display = 'none'
      showToast('[OK] PIN saved! Welcome, ' + worker.name + '!', 'success')
      await initMain()
    } else {
    ...
    if (data.success) {
      ...
      const notifyLine = notifyErrors.length === 0
        ...
        : `<p style="font-size:12px;color:#b45309;background:#fefce8;border:1px solid #fde68a;border-radius:10px;padding:10px;margin:0 0 20px;line-height:1.5;text-align:left">
            ...
            <strong>Note:</strong> ${emailOk ? '[OK] Email sent.' : '[!]? Email not sent (check admin email or Resend key in Settings).'} 
            ${smsOk ? '[OK] SMS sent.' : '[!]? SMS not sent (check admin phone & Twilio credentials in Settings).'}
           </p>`
...
function renderDispatchBanner() {
  ...
  banner.innerHTML = `
    ...
    <div class="flex-1 min-w-0">
      ...
      <p class="text-blue-400 text-xs mt-1">Tap Clock In -- your job is pre-selected [v]</p>
    </div>
...
function openJobModal() {
  ...
  if (gpsEl) {
    gpsEl.textContent = currentLat
      ? `[v] Location ready (?${currentLat.toFixed(3)}...)`
      : 'Getting location...'
  }
...
async function calcPickupETA() {
  ...
  _etaCalcTimer = setTimeout(async () => {
    ...
    try {
      ...
      if (!destCoords) { etaContent.innerHTML = '<span class="text-amber-600">[!]? Could not find store address on map</span>'; return }

      // Geocode return / job site address
      const returnCoords = await geocodeAddress(returnVal)
      if (!returnCoords) { etaContent.innerHTML = '<span class="text-amber-600">[!]? Could not find return job site on map</span>'; return }
      ...
      if (leg1 === null || leg2 === null) {
        etaContent.innerHTML = '<span class="text-amber-600">[!]? Could not calculate route -- check addresses</span>'
        return
      }
      ...
      etaContent.innerHTML = '<span class="text-amber-600">[!]? Could not calculate route</span>'
    }
...
async function respondDispatch(id, response) {
  ...
  try {
    ...
    const data = await res.json()
    if (data.success) {
      const labels = { accepted: '[OK] Job accepted!', declined: 'Job declined.', arrived: '? Marked as arrived!' }
